...
def create_artist_submission():
    ...
    try:
        artist = Artist(name=data['name'], city=data['city'], 
            state=data.get('state'), phone=data.get('genres'), genres=request.form.getlist('genres'), image_link=data.get('image_link'),
    ...
    if not error:
        ...
        flash('An error occurred. Artist ' + data['name'] + ' could not be listed.') # TODO: on unsuccessful db insert, flash an error instead.
...
def shows():
    ...
    for show in shows:
        show.artist_name = Artist.query.filter_by(id=show.artist_id).all()[0].name
        show.venue_name = Venue.query.filter_by(id=show.venue_id).all()[0].name
        show.start_time = str(show.start_time)

    print(shows[0].artist_name, shows[0].venue_name)
...
def create_show_submission():
    ...
    if not error:
        ...
        flash('An error occurred. Show could not be listed.')    # TODO: on unsuccessful db insert, flash an error instead.
